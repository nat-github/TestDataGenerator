...
# Make the parent project importable when launched as `python -m mcp_server.server`
...
def infer_relationships(
    ...
    # Run the existing main.py helper to write the reviewable YAML
    from main import _write_reviewable_yaml
    out_path = Path(config_output)
    out_path.parent.mkdir(parents=True, exist_ok=True)
    _write_reviewable_yaml(out_path, tables, existing, inferred)
...
def mock_init(
    ...
    """
    from main import _detect_mock_source_type
